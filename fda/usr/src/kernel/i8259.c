...
/*    if ((*hook->handler)(hook)) irq_actids[hook->irq] &= ~hook->id; */
      hook_handler_res = (*hook->handler)(hook) ;                    /* PP */
      if (hook_handler_res) irq_actids[hook->irq] &= ~hook->id;      /* PP */ 

#if 1
      intr_disable() ;                                               /* PP */
      plotear(proc_ptr, (hook->id << 16)         |                   /* PP */ 
	                    (hook_handler_res << 12) |                   /* PP */
                        (hook->irq << 8)         | opINTHND) ;       /* PP */
      intr_enable() ;                                                /* PP */  
#endif
      hook = hook->next;
  }
