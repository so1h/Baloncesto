 ...
 */
  ...
  switch(function) {                                                     /* PP */
  case SENDREC:      
      plotear((struct proc *)m_ptr, (src_dst_e << 16) | opSENDREC) ;     /* PP */
      break ;
  case SEND:      
      plotear((struct proc *)m_ptr, (src_dst_e << 16) | opSEND) ;        /* PP */
      break ;
  case RECEIVE:      
      plotear((struct proc *)m_ptr, (src_dst_e << 16) | opRECEIVE) ;     /* PP */
      break ;
  case NOTIFY:      
      plotear((struct proc *)m_ptr, (src_dst_e << 16) | opNOTIFY) ;      /* PP */
      break ;
  case ECHO:      
      plotear((struct proc *)m_ptr, (src_dst_e << 16) | opECHO) ;        /* PP */
      break ;
  default : ;
  }
 ...
 */
  ...
  if ( (dst_ptr->p_rts_flags & (RECEIVING | SENDING)) == RECEIVING &&
	...
	caller_ptr->p_sendto_e = dst_e;
 	plotear(caller_ptr, opSTATUS) ;       /* informamos del cambio de estado */  
 ...
 */
  ...
  /* Call from task level, locking is required. */
      ...
      lock(0, "notify");
      plotear(NIL_PROC, (dst << 16) | opL_NOTIFY) ; /* PP */
      result = mini_notify(proc_addr(src), dst); 
      unlock(0);
  }
  return(result);
 ...
 */
  ...
  plotear(rp, opENQUEUE) ;                          /* PP */

  /* Determine where to insert to process. */
  sched(rp, &q, &front);

  plotear(rp, (q << 16) | (front << 8) | opSCHED) ; /* PP */
 ...
 */
  ...
  for (rp=BEG_PROC_ADDR; rp<END_PROC_ADDR; rp++) {
      ...
      plotear((struct proc *)rp, opBALANCE) ;                     /* PP */	  
...
/* Safe gateway to mini_send() for tasks. */
  int result;
  lock(2, "send");
  plotear((struct proc *)m_ptr, (dst_e << 16) | opL_SEND) ; /* PP */
